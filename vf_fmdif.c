...
static int config_output(AVFilterLink *outlink)
{
    ...
    outlink->time_base = av_mul_q(ctx->inputs[INPUT_MAIN]->time_base, (AVRational){1, 2});
    outlink->w             = ctx->inputs[INPUT_MAIN]->w;
    outlink->h             = ctx->inputs[INPUT_MAIN]->h;

    if(s->mode & 1)
        outlink->frame_rate = av_mul_q(ctx->inputs[INPUT_MAIN]->frame_rate,
                                    (AVRational){2, 1});
    else
        outlink->frame_rate = ctx->inputs[INPUT_MAIN]->frame_rate;
...
const AVOption fmdif_options[] = {
    ...
    { "chroma",  "set whether or not chroma is considered in the combed frame decision", OFFSET_FMDIF(chroma),  AV_OPT_TYPE_BOOL,{.i64= 0},  0,    1, FLAGS },
    ...
    { "blocky",  "set the y-axis size of the window used during combed frame detection", OFFSET_FMDIF(blocky),  AV_OPT_TYPE_INT, {.i64=16},  4, 1<<9, FLAGS },
    { "combpel", "set the number of combed pixels inside any of the blocky by blockx size blocks on the frame for the frame to be detected as combed", OFFSET_FMDIF(combpel), AV_OPT_TYPE_INT, {.i64=100}, 0, INT_MAX, FLAGS },
